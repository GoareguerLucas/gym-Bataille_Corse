...
class QLearn:
    ...
    def __init__(self, action_space, alpha=0.1, gamma=0.9, epsilon=0.1):
        ...
        self.alpha = alpha
        self.gamma = gamma
        self.actions = action_space  # collection of choices
...
class AgentSimple(QLearn):
	"""
	Un agent qui utilise le Q-learning pour apprendre à jouer en regardant 
	seulement la valeur des quatres dernière cartes jouées et le tour de jeu
	"""
	def __init__(self, action_space):
		super(self.__class__, self).__init__(action_space)
	...
	def observation_reduce(self,observation):
		...
		if firsts_cards != None:
			for i in range(len(firsts_cards)):
				state[i+1] = firsts_cards[i].value
